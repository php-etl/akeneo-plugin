...
//        'referenceEntityMediaFile' => [], // no support
    ...
    public function getConfigTreeBuilder(): Config\Definition\Builder\TreeBuilder
    {
        ...
        $builder->getRootNode()
            ...
            ->end()
            ->validate()
                ->ifTrue(fn ($data) => !array_key_exists('conditional', $data) && array_key_exists('code', $data) && array_key_exists('type', $data) && !in_array($data['type'], ['attributeOption', 'referenceEntityRecord']))
                ->thenInvalid('The code option should only be used with the "attributeOption", "referenceEntityRecord" endpoints.')
            ->end()
            ->validate()
    ...
    private function getConditionalTreeBuilder(): Config\Definition\Builder\TreeBuilder
    {
        ...
        $builder->getRootNode()
            ...
            ->arrayPrototype()
                ...
                ->end()
                ->validate()
                    ->ifTrue(fn ($data) => array_key_exists('code', $data) && array_key_exists('type', $data) && !in_array($data['type'], ['attributeOption', 'referenceEntityRecord']))
                    ->thenInvalid('The code option should only be used with the "attributeOption", "referenceEntityRecord" endpoint.')
                ->end()
                ->validate()
