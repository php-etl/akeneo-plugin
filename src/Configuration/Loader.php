...
final class Loader implements PluginConfigurationInterface
{
    ...
    public function getConfigTreeBuilder(): \Symfony\Component\Config\Definition\Builder\TreeBuilder
    {
        ...
        $builder->getRootNode()
            ...
            ->end()
            ->validate()
                ->ifTrue(fn ($data) => \array_key_exists('reference_entity_attribute_option', $data)
                    && \array_key_exists('type', $data)
                    && !\in_array($data['type'], ['referenceEntityAttributeOption'], true))
                ->thenInvalid('The reference_entity_attribute_option option should only be used with the "referenceEntityAttributeOption" endpoint.')
            ->end()
            ->validate()
                ->ifTrue(fn ($data) => !\array_key_exists('reference_entity_attribute_option', $data)
                    && \array_key_exists('type', $data)
                    && \in_array($data['type'], ['referenceEntityAttributeOption'], true))
                ->thenInvalid('The reference_entity_attribute_option option should be used with the "referenceEntityAttributeOption" endpoint.')
            ->end()
            ->validate()
